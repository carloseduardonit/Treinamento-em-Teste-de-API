...
``` js
    ...
    pm.test("Resposta e sucesso",()=>{
        pm.response.json().sucess == true
    });
    pm.test("Verificar se o primeiro funcionario tem o ID '5f6d127156b0219cac2f5221'", function () {
        pm.expect(pm.response.json().data[0].id).to.eql("5f6d127156b0219cac2f5221");
    });
    pm.test("Verificar se o primeiro funcionario tem o nome 'Airi Nagano'", function () {
        pm.expect(pm.response.json().data[0].name).to.eql("Airi Nagano");
    });
...
{{#if error}}
    <div class="error-message">{{error}}</div>
{{else}}
    <h3>People List ({{count}} results)</h3>
...
function constructVisualizerPayload() {
    ...
    if (!response.data || !Array.isArray(response.data) || response.data.length === 0) {
        return { error: "No people data available in the response." };
    }

    // Processamento dos dados
    var people = response.data.map(function(person) {
        var gamesList = Array.isArray(person.worked_on)
            ? person.worked_on.map(url => url.split('/').pop()).join(', ')
            : "N/A";

        return {
            id: person.id || "N/A",
            name: person.name || "N/A",
            gamesCount: person.worked_on ? person.worked_on.length : 0,
            games: gamesList
        };
    });
    ...
    return {
        people: people,
        count: response.count || people.length
    };
}
...
``` html
onst responseJson = pm.response.json();
...
``` js
    ...
    pm.test("Validar a estrutura json do personagem", function () {
        var json = pm.response.json();
        // O status 200 foi retornado e o campo success é true
        pm.expect(json.success).to.be.true;
        // O campo count é maior ou igual a zero
        pm.expect(json.count).to.be.at.least(0);
        // O campo data é um array
        pm.expect(json.data).to.be.an('array');
        // O campo data não está vazio
        pm.expect(json.data).to.not.be.empty;
        // O campo data contém objetos com as propriedades esperadas
        pm.expect(json.data[0]).to.have.property('name');
        pm.expect(json.data[0]).to.have.property('gender');
        pm.expect(json.data[0]).to.have.property('race');
        pm.expect(json.data[0]).to.have.property('appearances');
        pm.expect(json.data[0]).to.have.property('id');
        // O campo appearances é um array   
        pm.expect(json.data[0].appearances).to.be.an('array');
    });
    var app = pm.response.json().data[0].appearances[0]
    let resquest = {
        url: app,
        method: "GET",
    }
...
- Payload de visualização personalizado para exibir os dados dos personagens em uma tabela formatada, juntamente com gráficos de distribuição por gênero e raça.
...
- Implementação do codigo para teste de  API:
```Js
...
``` js
    ...
    var nome = pm.environment.get("nomePersonagem")
    pm.test("Verifica se o nome do personagem é igual ao esperado", function () {
        pm.expect(data.name).to.eql(nome);
    });
    pm.test("Verifica se o ID do personagem é igual ao esperado", function () {
        pm.expect(data.id).to.eql("5f6d186f246bd9a0809d6116");
    });
    pm.test("O sexo do personagem esta Null",()=>{
        pm.expect(data.gender).to.not.eql("Female");
        pm.expect(data.gender).to.not.eql("Male");
        pm.expect(data.gender).to.eql(null);
        pm.expect(data.gender).to.null
    });
    pm.test("A corrida do personagem esta Null",()=>{
        pm.expect(data.race).to.eql(null);
        pm.expect(data.race).to.null
    });
    var app = data.appearances[0]
    let resquest = {
        url: app,
        method: "GET",
    }
    pm.sendRequest(resquest, function (err, response) {
        if (err) {
            ...
            pm.test("O ID do jogo é 5f6ce9d805615a85623ec2c0", function () {
                pm.expect(data.id).to.eql("5f6ce9d805615a85623ec2c0");
            });
            pm.test("O nome do jogo é The Legend of Zelda: Oracle of Seasons", function () {
                pm.expect(data.name).to.eql("The Legend of Zelda: Oracle of Seasons");
            });
            pm.test("O nome do desenvolvedor é Capcom", function () {
                pm.expect(data.developer).to.eql("Capcom");
            });
            pm.test("O nome do publicado é Nintendo",()=>{
                pm.expect(data.publisher).to.eql("Nintendo");
            });
            pm.test("A data de publicação do game e May 14, 2001",()=>{
                pm.expect(data.released_date).to.eql(" May 14, 2001");
            });
        }
    });
```
...
``` js
    var jsonRespose = pm.response.json();
    var data = jsonRespose.data[1];
    var urlGame = data.appearances[0];
    var game ={
        url: urlGame,
        method: 'GET'
    }
    var urlMasmorras = data.dungeons[0];
    var masmorra ={
        url: urlMasmorras,
        method: "GET"
    }
    pm.test("Verificar se a solitação foi realizada com sucesso", function () {
        console.clear()
        console.log(jsonRespose);
        pm.response.to.have.status(200);
        pm.expect(jsonRespose.success).to.eql(true);
        pm.expect(jsonRespose.success).to.true;
        pm.expect(jsonRespose.count).to.eql(20);
        pm.expect(jsonRespose.count).to.be.above(10,"Não tem  mais  de 10 elementos");
        pm.expect(jsonRespose.data).to.be.an("array");
    });
    pm.test("Verificar se a estrutura do Bosses",()=>{
        console.log(data)
        pm.expect(data).to.have.property("id");
        pm.expect(data).to.have.property("name");
        pm.expect(data).to.have.property("description");
        pm.expect(data).to.have.property("appearances"); 
        pm.expect(data.appearances).to.be.an("array","Não é um Array");
            pm.expect(data).to.have.property("dungeons"); 
            pm.expect(data.dungeons).to.be.an("array","Não é um Array");	
        });
        pm.test("Verificar ser nome do Chefe é 'Amy'", function () {
            pm.expect(data.name).to.eql("Amy");
        });
    pm.sendRequest(game, function (err, response) {
        if (err) {
            console.log(err);
        } else {
            response = response.json().data;
            console.log(response);
            pm.test("Verificar a estrutura  do game esta correta",()=>{
                pm.expect(response).to.have.property("id")
                pm.expect(response).to.have.property("name")
                pm.expect(response).to.have.property("description")
                pm.expect(response).to.have.property("developer")
                pm.expect(response).to.have.property("publisher")
                pm.expect(response).to.have.property("released_date")
            });
            pm.test("Verificar se o nome do jogo é 'The Legend of Zelda: Ocarina of Time'", function () {
                pm.expect(response.name).to.eql("The Legend of Zelda: Ocarina of Time");    
            });
            pm.test("Verificar se o ID do jogo é '5f6ce9d805615a85623ec2ba'", function () {
                pm.expect(response.id).to.eql("5f6ce9d805615a85623ec2ba");    
            });
            pm.test("A descrição do jogo contem 'Legend of Zelda series that was visually ...'",()=>{
                pm.expect(response.description).to.include("Legend of Zelda series that was visually");    
            });
            pm.test("Verificar o nome desenvolvedor do game é Nintendo EAD",()=>{
                pm.expect(response.developer).to.eql("Nintendo EAD");    
            });
            pm.test("Verificar o nome publicado do game é Nintendo",()=>{
                pm.expect(response.publisher).to.eql("Nintendo");    
                pm.expect(response.publisher).to.include("Nin");
            });
            pm.test("Verificar a data do release é 'November 23, 1998'",()=>{
                pm.expect(response.released_date).to.eql(" November 23, 1998");    
                pm.expect(response.released_date).to.include("1998");
                pm.expect(response.released_date).to.include("November")   
            });
        }
    });
    pm.sendRequest(masmorra, function (err, response) {
        if(err){
            console.log(err);
        } else {
            response = response.json().data;
            console.log(response);
            pm.test("Verificar a estrutura da masmorra esta correta",()=>{
                pm.expect(response).to.have.property("id");
                pm.expect(response).to.have.property("name");
                pm.expect(response).to.have.property("description");
                pm.expect(response).to.have.property("appearances"); 
                pm.expect(response.appearances).to.be.an("array","Não é um Array");
            });
            pm.test("Verificar se o nome da masmorra é 'Forest Temple'", function () {
                pm.expect(response.name).to.eql("Forest Temple");    
            });
            pm.test("Verificar se o ID da masmorra é '5f6d158a3dd55d9ed2d1c1a2'", function () {
                pm.expect(response.id).to.eql("5f6d158a3dd55d9ed2d1c1a2");    
            });
            pm.test("Verificar se a descrição da masmorra contém 'The Forest Temple is a recurring dungeon in The Legend of Zeld'", function () {
                pm.expect(response.description).to.include("The Forest Temple is a recurring dungeon in The Legend of Zeld");    
            });
        }
    });
```
...
#### Resultado Obtido:
- Retorna uma lista com todos os chefes da série The Legend of Zelda.
![Obter todos os chefes](Chefes/Obtertodososchefes.gif)
...
```
##### Resultado Obtido:
- Retorna os detalhes do chefe correspondente ao ID fornecido.
![Obter chefe por ID](Chefes/Obterchefeporid.gif)
...
``` js
    ...
    // Verificar o ID
    pm.test("2 - ID do lugar é o mesmo", function () {
        var Lid = "5f6e93a7cbf4202fbde22501";
        pm.expect(id).to.eql(Lid);
    });
    pm.test("3 - Nome do lugar é o mesmo", function () {
        var Lname = pm.environment.get("NomeChefe");
    ...
    pm.sendRequest(urlGame,function(err, res){
        ...
        if(count > 0){
            pm.test("4.1 - Lugar aparece no jogo", function () {
                let nome = res.json().data.name;
                pm.expect(nome).to.eql("The Legend of Zelda: A Link Between Worlds");
            });
            pm.test("4.2 - O Lugar foi desevolvido pela empresa ",function(){
                let developer = res.json().data.developer;
            ...
            pm.test("4.4 - O lugar tem Data de Lançamento", function(){
                let releaseDate = res.json().data.released_date;
                pm.expect(" November 22, 2013").to.eql(releaseDate);
            });
        } else {
            pm.test("4.0 - Lugar não aparece no jogo", function () {
                pm.expect(Games.length).to.eql(0);
...
###### Resultado Obtido:
- Retorna os chefes que correspondem ao nome fornecido.
![Obter chefes por nome](Chefes/Obterchefespornome.gif)
...
``` js
    ...
    pm.test("Verificar a estrutura da Masmorra",()=>{
        console.clear();
        console.log(data);
        ...
        pm.expect(data).to.have.property("description");
        pm.expect(data).to.have.property("appearances");
        pm.expect(data.appearances).to.be.an("array");
    });
    pm.test("Verificar o ID da Masmorra",()=>{
        pm.expect(data.id).to.equal("5f6d158a3dd55d9ed2d1c083");
    });
    pm.test("Verificar o nome da Masmorra",()=>{
        pm.expect(data.name).to.equal("Abyss of Agony");
    });
        pm.test("Verificar a descrição da Masmorra",()=>{
        pm.expect(data.description).to.include("Abyss of Agony is a Level in Tri Force");
    });
    pm.sendRequest(game, function (err, response) {
        var gameResponse = response.json();
        var gameData = gameResponse.data;
        console.log(gameResponse)
        if(err){
            console.log(err);
        } else {
            pm.test("Verificar a estrutura do jogo",()=>{
                pm.expect(gameResponse).to.have.property("success");
                pm.expect(gameResponse).to.have.property("data");
                pm.expect(gameData).to.be.an("object");
                pm.expect(gameData).to.have.property("id");
                pm.expect(gameData).to.have.property("name");
                pm.expect(gameData).to.have.property("description");
                pm.expect(gameData).to.have.property("developer");
                pm.expect(gameData).to.have.property("publisher");
                pm.expect(gameData).to.have.property("released_date");
            });	
            pm.test("Verificar se requisição foi um sucesso",()=>{
                pm.expect(gameResponse.success).to.equal(true);	
                pm.expect(gameResponse.success).to.true;
            });
            pm.test("Verificar o ID do jogo",()=>{
                pm.expect(gameData.id).to.equal("5f6ce9d805615a85623ec2ce");	
            });
            pm.test("Verificar o nome do jogo",()=>{
                pm.expect(gameData.name).to.equal("The Legend of Zelda: Tri Force Heroes");	
            });
            pm.test("Verificar  na descrição do jogo contem a frase: 'Tri Force Heroes is the eighteenth'",()=>{
                pm.expect(gameData.description).to.include("Tri Force Heroes is the eighteenth");	
            });
            pm.test("Verificar o desenvolvedor do jogo",()=>{
                pm.expect(gameData.developer).to.equal("Nintendo");	
            });
            pm.test("Verificar a editora do jogo",()=>{	
                pm.expect(gameData.publisher).to.equal("Nintendo");	
            });
            pm.test("Verificar a data de lançamento do jogo",()=>{
                pm.expect(gameData.released_date).to.equal(" October 23, 2015");	
            });	
        }
    });
```
...
##### Resultado Obtido:
- Retorna uma lista com todas as masmorras da série The Legend of Zelda.
![Obter todas as masmorras](Masmorras/Obtertodosasmasmorras.gif)
...
```
##### Resultado Obtido:
- Retorna os detalhes da masmorra correspondente ao ID fornecido.
![Obter masmorra por ID](Masmorras/Obtermasmorraporid.gif)
...
``` js
    ...
    // Verificar o ID
    pm.test("2 - ID do lugar é o mesmo", function () {
        var Lid = "5f6d158a3dd55d9ed2d1c08c";
        pm.expect(id).to.eql(Lid);
    });
    pm.test("3 - Nome do lugar é o mesmo", function () {
    ...
    pm.sendRequest(urlGame,function(err, res){
        ...
        if(count > 0){
            pm.test("4.1 - Lugar aparece no jogo", function () {
                let nome = res.json().data.name;
                pm.expect(nome).to.eql("The Legend of Zelda: Phantom Hourglass");
            });
            pm.test("4.2 - O Lugar foi desevolvido pela empresa ",function(){
                let developer = res.json().data.developer;
                pm.expect(developer).to.eql("Nintendo EAD");
            ...
            pm.test("4.4 - O lugar tem Data de Lançamento", function(){
                let releaseDate = res.json().data.released_date;
                pm.expect(' October 1, 2007').to.eql(releaseDate);
            });
        } else {
            pm.test("4.0 - Lugar não aparece no jogo", function () {
                pm.expect(Games.length).to.eql(0);
...
```
##### Resultado Obtido:
- Retorna as masmorras que correspondem ao nome fornecido.
![Obter masmorras por nome](Masmorras/Obtermasmorraspornome.gif)
...
##### Resultado Obtido:
- Retorna uma lista com todos os lugares da série The Legend of Zelda.
![Obter todos os lugares](Lugares/Obtertodososlugares.gif)
...
##### Resultado Obtido:
- Retorna os detalhes do lugar correspondente ao ID fornecido.
![Obter lugar por ID](Lugares/Obterlugarporid.gif)
...
##### Resultado Obtido:
- Retorna os lugares que correspondem ao nome fornecido.
![Obter lugares por nome](Lugares/Obterlugarespornome.gif)
...
##### Resultado Obtido:
- Retorna uma lista com todos os itens da série The Legend of Zelda.
![Obter todos os itens](Itens/Obtertodosositens.gif)
...
``` js
    ...
    var Nome = pm.response.json().data.name; // Nome do item
    const gameIds = pm.response.json().data.games; // Array de IDs de jogos
    var URLGame = gameIds[0];
    pm.test(" 1 - O Status e  igual a 200",function(){
        pm.expect(pm.response.code).to.eql(200);
    });
    pm.test("2 - O nome do item é igual a Ancient Arrow", function(){
        var Lnome = "Ancient Arrow";
        pm.expect(Nome).to.eql(Lnome);
    });
    pm.test("3 - O ID do item é igual a 5f6e9a9dfee1a5347127c76f", function () {
        var Lid = pm.environment.get("IDItem");
        pm.expect(ID).to.eql(Lid);
    });
    pm.sendRequest(URLGame,function(err, res){
...
##### Resultado Obtido:
- Retorna os detalhes do item correspondente ao ID fornecido.
![Obter item por ID](Itens/Obteritemporid.gif)
...
##### Resultado Obtido:
-  Retorna os itens que correspondem ao nome fornecido.
![Obter itens por nome](Itens/Obteritenspornome.gif)
